...
> **⚠ 项目名固定使用"代码审计报告"**。"shit" 只是 skill 内部的项目代号，对外报告、HTML 标题、文件名等所有用户可见的地方**禁止出现** "shit" 字样。

全项目代码审计技能 —— 用多 sub-agent 并行审查整个代码库，找出架构违规、可维护性问题、业务逻辑散乱等"屎山"问题，输出结构化的 HTML 报告。
